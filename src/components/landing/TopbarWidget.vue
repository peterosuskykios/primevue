...
<template>
    <div class="flex items-center justify-between relative lg:static py-12 px-6 mx-0 md:px-16 lg:px-20 lg:py-12 lg:mx-20">
        ...
        <i v-styleclass="{ selector: '@next', enterFromClass: 'hidden', leaveToClass: 'hidden', hideOnOutsideClick: true }" class="pi pi-bars !text-4xl cursor-pointer block md:hidden text-surface-700 dark:text-surface-100"></i>

        <div class="items-center flex-grow-1 hidden md:flex absolute md:static w-full md:px-0 z-30 shadow md:shadow-none animate-fadein bg-surface-100 dark:bg-surface-950" :class="{ hidden: isHidden }" :style="{ top: '80px', right: '0%' }">
            <ul class="list-none p-4 md:p-0 m-0 ml-auto flex md:items-center select-none flex-col md:flex-row cursor-pointer -">
                <a class="cursor-pointer !block md:!hidden text-primary-500 font-medium leading-normal absolute" style="top: 1.5rem; right: 2rem" @click="toggleHidden">
